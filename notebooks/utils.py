...
class ImageLoader:
    ...
    def load_image(image_filepath, segment_filepath, spacing, size):
        ...
        np_image_resampled = ImageLoader.sitk_to_numpy(image_resampled)
        np_segment_resampled = ImageLoader.sitk_to_numpy(segment_resampled, normalize=False).astype(np.uint8)
        ...
        crop_origin = seg_center_resampled[::-1] - crop_size[::-1] // 2
        crop_ending = seg_center_resampled[::-1] + crop_size[::-1] // 2
        ...
        np_image_final = np.zeros(shape=crop_size)
        np_segment_final = np.zeros(shape=crop_size, dtype=np.uint8)
...
class DatasetCreator:
    ...
    def __init__(self, dataset_path, dim, resampling_spacing):
